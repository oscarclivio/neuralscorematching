...
for seed in [k*1000 for k in range(2,11+1)]:
    for exp_num in range(1, 50 + 1):
        ...
        train_data = News(exp_num, dataset="train_val", tensor=False, data_folder="../../../data")
        test_data = News(exp_num, dataset="test", tensor=False, data_folder="../../../data")
        ...
        later = datetime.now()
        diff = later - now
        print("Matching on News dataset {}, seed {}, took {} seconds".format( exp_num, seed, diff.seconds))

df_res = pd.DataFrame(res)

df_res.to_csv('../../../outputs/matching_benchmark_outputs/news_stability.csv')
