...
def matching_benchmarks(models, data, data_to_add=None, t_to_add=0, programme=None, metrics=None, seed=None):
    x, y, t, mu0, mu1, cate_true, ps_gt = data[:]
...
for seed in [k*1000 for k in range(2,11+1)]:
    for exp_num in range(1, 100 + 1):
        now = datetime.now()

        train_data = ACIC2016(SETTING, exp_num, dataset="train_val", tensor=False, path_data="../../../data")
        test_data = ACIC2016(SETTING, exp_num, dataset="test", tensor=False, path_data="../../../data")
        ...
        for ps_pred in ["propensity_x", "propensity_x_pca"]:
            res.update({ps_pred + '_calibration_error_in': res.get(ps_pred + '_calibration_error_in', []) + [np.mean(np.abs(scores[ps_pred] - scores['ps_gt']))]})
        ...
        for ps_pred in ["propensity_x", "propensity_x_pca"]:
            res.update({ps_pred + '_calibration_error_out': res.get(ps_pred + '_calibration_error_out', []) + [np.mean(np.abs(scores[ps_pred] - scores['ps_gt']))]})
        later = datetime.now()
        diff = later - now
        print("Matching on ACIC2016 setting {}, dataset {}, seed {}, took {} seconds".format(SETTING, exp_num, seed, diff.seconds))

df_res = pd.DataFrame(res)

df_res.to_csv('../../../outputs/matching_benchmark_outputs/acic2016_stability.csv')
